...
export default function Properties() {
  ...
  return (
    <div className="pt-24 pb-20 bg-slate-50 min-h-screen animate-fade-in">
      ...
      {selectedProperty && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSelectedProperty(null)}>
          <div className="bg-white w-full max-w-5xl rounded-2xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center px-6 py-4 border-b bg-slate-900 text-white">
              <div><h3 className="text-2xl font-serif font-bold">{selectedProperty.title}</h3><p className="text-sm text-slate-300">{selectedProperty.location} • {selectedProperty.price}</p></div>
              <button onClick={() => setSelectedProperty(null)} className="p-2 rounded-full hover:bg-white/10"><X /></button>
            </div>
            <div className="p-6 grid md:grid-cols-5 gap-6">
              <div className="md:col-span-3 grid grid-cols-2 gap-4">{selectedProperty.gallery.map((img) => <img key={img} src={img} alt={selectedProperty.title} className="w-full h-48 object-cover rounded-xl" />)}</div>
              <form className="md:col-span-2 space-y-3 bg-slate-50 p-5 rounded-xl border" onSubmit={handleSubmit}>
                <h4 className="font-semibold text-lg">Book Site Visit</h4>
